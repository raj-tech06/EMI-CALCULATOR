...
function updateEMI() {
  ...
  const interestRate = Number.parseFloat(interestRateInput.value) / 100 / 12 // मासिक ब्याज दर
  const loanTerm = Number.parseInt(loanTermInput.value) * 12 // महीनों में अवधि
...
function updateSliderFromInput(input, slider, display, prefix = "", suffix = "") {
  ...
  if (isNaN(value)) {
    value = Number.parseFloat(slider.min)
  }
  value = Math.max(Number.parseFloat(slider.min), Math.min(Number.parseFloat(slider.max), value))
  slider.value = value
  input.value = value
  display.textContent = prefix + value.toLocaleString("en-IN") + suffix
  updateEMI()
...
loanAmountInput.addEventListener("input", () =>
  updateInputFromSlider(loanAmountInput, loanAmountNumberInput, loanAmountDisplay, "₹"),
)
interestRateInput.addEventListener("input", () =>
  updateInputFromSlider(interestRateInput, interestRateNumberInput, interestRateDisplay, "", "%"),
)
loanTermInput.addEventListener("input", () =>
  updateInputFromSlider(loanTermInput, loanTermNumberInput, loanTermDisplay, "", " वर्ष"),
)

loanAmountNumberInput.addEventListener("input", () =>
  updateSliderFromInput(loanAmountNumberInput, loanAmountInput, loanAmountDisplay, "₹"),
)
interestRateNumberInput.addEventListener("input", () =>
  updateSliderFromInput(interestRateNumberInput, interestRateInput, interestRateDisplay, "", "%"),
)
loanTermNumberInput.addEventListener("input", () =>
  updateSliderFromInput(loanTermNumberInput, loanTermInput, loanTermDisplay, "", " वर्ष"),
)
